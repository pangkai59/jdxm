...
public class RMBUtils {
    ...
    /**
     * 将数字转换为财务记账的格式(保留两位小树)
     *
     * @param in 金额
     * @return 财务记账格式
     */
    public static String toFinancial(Double in) {
    ...
    /**
     * 将数字转换为财务记账的格式(保留两位小数，如果小树为'.00',那么就将小数部分去掉)
     *
     * @param in 金额
     * @return 财务记账格式
     */
    public static String toFinancialNoZero(Double in) {
        DecimalFormat df = new DecimalFormat("#,###.00");
        String amount = df.format(in);

        amount = amount.replaceAll("\\.0{2,}","");
    ...
    public static String convert(Double amount) throws IllegalArgumentException {
        if (amount == 0) {
            return "零";
        }

        String s = new DecimalFormat("#.00").format(amount);
        return convert(s);
    }
